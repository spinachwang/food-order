...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.live:
        ...
        try:
            ...
            # invalid_key_401 fixture 会触发; 视为成功 (验证了错误码)
            steps.append(
                {
                    "step": "error_path",
                    "error_code": exc.code,
            ...
            _print_step("error_path", steps[-1])
            return 0
        except AmapQuotaExceededError as exc:
            steps.append(
                {
    ...
    # ---- 验收点 ----
    if main_result["mode"] == "mock":
        # F030 §2: 不少于 3 家 → partial=False (取决于 fixture)
        expected_count = main_result["raw_count"]
        if expected_count >= 3:
            overall_pass = overall_pass and main_result["partial"] is False
        else:
            overall_pass = overall_pass and main_result["partial"] is True
        # F030 §2: 距离 > 3km 被剔除
        for r in main_result["result"]["restaurants"]:
            assert isinstance(r, dict)
            d = r.get("distance_meters")
            assert isinstance(d, int) and d <= 3000, f"distance > 3000m 不应进入结果: {r}"
