...
CLI：`python scripts/japanese.py "<消息>"`，验证日料专家 build_prompt +
parse_output 两条契约路径 + 与西式快餐（F020）注册表共存不冲突。

设计目的：开发 / 调试阶段快速验证 F018 §2 / §6 验收点。

- Phase 1 stub `run()` 抛 `NotImplementedError`，所以脚本不调真 LLM，只走
  专家层的两条核心契约：
  1. `build_prompt`：用户输入 → prompt 字面量（验证菜系词 / 代表菜 /
     三大代表 / 与西式快餐边界澄清是否落到 prompt）
  2. `parse_output`：模拟 LLM 回包 → `CuisineExpertOutput`（happy path +
     非法 JSON fallback）

- `--parallel-check` 验证与西式快餐（F020）注册表共存 + prompt_fragment
  互不污染，对应 §6 集成验收点"与西式快餐并行时不冲突"。
...
用法：
    ...
    # 1. Happy path: 用户输入"想吃寿司" → build_prompt 含"日料"
    python scripts/japanese.py "想吃寿司"

    # 2. 三大代表场景: 用户输入"想吃刺身"
    python scripts/japanese.py "想吃刺身"

    # 3. 边界场景: 用户输入"想吃便利店便当" → 验证边界澄清已落到 prompt
    python scripts/japanese.py "想吃便利店便当"

    # 4. parse_output happy path（注入模拟 LLM 回包）
    python scripts/japanese.py --canned-json \\
        '{"conclusion":"今天适合来份寿司","keywords":["日料","寿司","刺身","和风"],"matched_allergies":[]}'

    # 5. parse_output fallback（非法 JSON）
    python scripts/japanese.py --canned-json "not json"

    # 6. 与西式快餐并行不冲突（注册表 + prompt 隔离）
    python scripts/japanese.py "今天都行" --parallel-check

    # 7. 性能硬上限（>2 秒报错退出）
    python scripts/japanese.py "想吃寿司" --max-elapsed-ms 2000
"""

from __future__ import annotations

# 允许从仓库根目录直接 `python scripts/japanese.py "..."` 而不需要 PYTHONPATH。
...
def _check_prompt_hits(prompt: str) -> dict[str, bool]:
    """检查 build_prompt 输出对 F018 §2 / §4 关键提示的命中情况。

    返回字段语义：True = 命中（提示词出现在 prompt 中），False = 缺失。
    """
    return {
        # §2 验收：菜系词覆盖（日料 / 日本料理 至少其一）
        "cuisine_term_riliao": "日料" in prompt,
        "cuisine_term_ribenliaoli": "日本料理" in prompt,
        # §2 验收：三大代表（寿司 / 刺身 / 拉面）
        "signature_sushi": "寿司" in prompt,
        "signature_sashimi": "刺身" in prompt,
        "signature_ramen": "拉面" in prompt,
        # §2 验收：与西式快餐（F020）的边界
        "distinguish_from_western_fastfood": "西式快餐" in prompt,
        "boundary_convenience_store": "便利店" in prompt or "便当" in prompt,
        # §4 风味词
        "flavor_hefeng": "和风" in prompt,
        # 用户原文注入
...
def _check_parallel_safety() -> dict[str, Any]:
    """F018 §6 集成验收：与西式快餐并行时不冲突。

    验证日料（F018）与西式快餐（F020）注册表共存 + prompt_fragment 互不污染。
    """
    japanese = CUISINE_REGISTRY.get("japanese")
    wf = CUISINE_REGISTRY.get("western_fastfood")

    coexists = japanese is not None and wf is not None
    fragments_differ = (
        coexists and JapaneseExpert.prompt_fragment != WesternFastfoodExpert.prompt_fragment
    )
    # 日料 fragment 不含西式快餐专属代表菜（避免 LLM 误推西式快餐）
    wf_signature_dishes = ("汉堡", "薯条", "炸鸡", "披萨")
    japanese_no_wf_dish_leak = all(
        dish not in JapaneseExpert.prompt_fragment for dish in wf_signature_dishes
    )
    # 西式快餐 fragment 不含日料专属代表菜
    japanese_signature_dishes = ("寿司", "刺身", "拉面", "天妇罗", "鳗鱼饭")
    wf_no_japanese_dish_leak = all(
        dish not in WesternFastfoodExpert.prompt_fragment for dish in japanese_signature_dishes
    )

    return {
        "both_registered": coexists,
        "fragments_differ": fragments_differ,
        "japanese_no_wf_dish_leak": japanese_no_wf_dish_leak,
        "wf_no_japanese_dish_leak": wf_no_japanese_dish_leak,
        "all_pass": all(
            [
                coexists,
                fragments_differ,
                japanese_no_wf_dish_leak,
                wf_no_japanese_dish_leak,
            ]
        ),
    }
...
def _run_build_prompt(user_message: str) -> dict[str, Any]:
    ...
    # F018 §2 验收点的关键最小集：
    # 菜系词至少 1 个 + 三大代表 (寿司 / 刺身 / 拉面) + 与西式快餐边界澄清 + 风味词"和风"
    section2_minimum = all(
        [
            hits["signature_sushi"],
            hits["signature_sashimi"],
            hits["signature_ramen"],
            hits["distinguish_from_western_fastfood"],
            hits["boundary_convenience_store"],
            hits["flavor_hefeng"],
            any(
                [
                    hits["cuisine_term_riliao"],
                    hits["cuisine_term_ribenliaoli"],
                ]
            ),
        ]
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="F018 日料专家手工试跑（默认走 build_prompt；--canned-json 走 parse_output）"
    )
    parser.add_argument(
        "message",
        nargs="?",
    ...
    parser.add_argument(
        "--parallel-check",
        action="store_true",
        help="附加运行：与西式快餐（F020）注册表共存 + prompt_fragment 隔离检查（§6 集成验收）",
    )
    parser.add_argument(
        "--max-elapsed-ms",
        type=int,
