...
- ~~是否需要"用户身份"概念？~~ **已决议 2026-08-30**：M1 用前端生成的匿名 UUID（`X-User-Id` 请求头 + cookie），**不做登录**，登录态推到 M2
- ~~`budget_lunch_max` 是否包含配送费？~~ **已决议 2026-08-30**：**包含配送费**（餐品 + 打包费 + 平台配送费的合计上限）
- ~~是否对单条 `user_feedback` 设去重窗口？~~ **已决议 2026-08-30**：M1 不做 feedback（无 `user_feedback` 表、无 `/feedback` 端点），整体延后到 M2；M2 再考虑去重